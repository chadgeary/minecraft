# Vendor AMI
data "aws_ami" "mc-vendor-ami-latest" {
  most_recent             = true
...
resource "aws_ami_copy" "mc-latest-vendor-ami-with-cmk" {
  name                    = "${var.name_prefix}-encrypted-ami-${random_string.mc-random.result}"
  description             = "KMS CMK-encrypted AMI of latest official vendor AMI"
  source_ami_id           = data.aws_ami.mc-vendor-ami-latest.id
